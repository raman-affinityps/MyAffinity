...
define( 'SITEORIGIN_THEME_VERSION' , '2.4.4' );
...
function my_show_extra_profile_fields( $user ) { ?>
	...
	<table class="form-table">
		...
		<tr>
			<th><label for="lender">Lender</label></th>

			<td>
				<input type="text" name="lender" id="lender" value="<?php echo esc_attr( get_the_author_meta( 'lender', $user->ID ) ); ?>" class="regular-text" /><br />
				<span class="description">Please enter the Lender name.</span>
			</td>
		</tr>
...
function my_save_extra_profile_fields( $user_id ) {
	...
	/* Copy and paste this line for additional fields. Make sure to change 'lender' to the field ID. */
	update_usermeta( $user_id, 'lender', $_POST['lender'] );
}

function custom_scripts() {
	wp_enqueue_style( 'jquery-ui-style', 'https://code.jquery.com/ui/1.11.4/themes/smoothness/jquery-ui.css' );
	wp_enqueue_style( 'daterangepicker', get_template_directory_uri() . '/js/daterangepicker.css' );
	
	wp_enqueue_script("jquery-ui",'https://code.jquery.com/ui/1.11.4/jquery-ui.min.js', array('jquery'), '1.0.0', true);
	wp_enqueue_script( 'responsiveTabs', get_template_directory_uri() . '/js/responsiveTabs.js', array(), '1.0.0', true );
	wp_enqueue_script( 'daterangepicker', get_template_directory_uri() . '/js/daterangepicker.js', array(), '1.0.0', true );
...
add_action( 'wp_ajax_analytics_dashboard', 'analytics_dashboard' );

function analytics_dashboard()
{	
    if ( isset($_REQUEST['apdb_startdate']) && isset($_REQUEST['apdb_enddate']) ) 
    {
        $_SESSION['apdb_startdate'] = date('Y-m-d', strtotime($_REQUEST['apdb_startdate']));
        $_SESSION['apdb_enddate']   = date('Y-m-d', strtotime($_REQUEST['apdb_enddate']));
            //echo '{"start": "' . $_REQUEST['apdb_startdate'] . '","end":"' . $_REQUEST['apdb_enddate'] . '"}';
    }
    require_once get_template_directory().'/includes/config.php';    
    $obj                = new AnalyticsData($ajax);    
    if($ajax==0)
    {
        /* Top Active Page Data */
        $otherOpt       = array( 'dimensions'=> 'ga:pagePath' );
        $usermatrix     = ['ga:users'];
        $obj->getGaData(ANALYTIC_DEFAULT_START_DATE, ANALYTIC_DEFAULT_END_DATE, $usermatrix,$otherOpt,'funnel',$ajax);

        $otherOpt       = array( 'dimensions'=> 'ga:pagePath,ga:nthDay' );
        $usermatrix     = ['ga:users'];
        $obj->getGaData(ANALYTIC_DEFAULT_START_DATE, ANALYTIC_DEFAULT_END_DATE, $usermatrix,$otherOpt,'linechartfunnel',$ajax);

       /* Device category (Desktop, Tablet, or Mobile.) */
        $otherOpt       = array( 'dimensions'=> 'ga:deviceCategory' );
        $usermatrix     = ['ga:pageviews'];
        $obj->getGaData(ANALYTIC_DEFAULT_START_DATE, ANALYTIC_DEFAULT_END_DATE,$usermatrix,$otherOpt,'semicircle',$ajax);

        /* User / New User */	
        $otherOpt       = array( 'dimensions'=> 'ga:nthDay' );
        $usermatrix     = ['ga:users','ga:newUsers'];
        $obj->getGaData(ANALYTIC_DEFAULT_START_DATE, ANALYTIC_DEFAULT_END_DATE, $usermatrix,$otherOpt,"linechart",$ajax);


        /* Per Minute */
        $otherOpt       = array( 'dimensions'=> 'ga:minute' );
        $usermatrix     = ['ga:pageviews'];
        $obj->getGaData(ANALYTIC_DEFAULT_START_DATE, ANALYTIC_DEFAULT_END_DATE, $usermatrix,$otherOpt,'linchartPagePerMinute',$ajax);
        
        /*Audience / Geo / Location*/
        $otherOpt       = array( 'dimensions'=> 'ga:country' );
        $usermatrix     = ['ga:visitors'];
        $obj->getGaData(ANALYTIC_DEFAULT_START_DATE, ANALYTIC_DEFAULT_END_DATE, $usermatrix,$otherOpt,'geoLocation',$ajax);
    ...
    else
    {
        ...
        switch ($functionname) 
        {
            ...
            case 2:
                ...
                break;
            case 3:
                $otherOpt   = array( 'dimensions'=> 'ga:deviceCategory','filters' => 'ga:pagePath==/#request-pricing' );
                $usermatrix = ['ga:users'];
                $obj->getGaData(ANALYTIC_DEFAULT_START_DATE, ANALYTIC_DEFAULT_END_DATE,$usermatrix,$otherOpt,'semicircle',$ajax);
                break;
            case 4:
            ...
            case 5:
                ...
                break;            
            case 6:
                $otherOpt   = array( 'dimensions'=> 'ga:region','filters' => 'ga:country==United%20States');
                $usermatrix = ['ga:sessions'];
                $obj->getGaData(ANALYTIC_DEFAULT_START_DATE, ANALYTIC_DEFAULT_END_DATE, $usermatrix,$otherOpt,'geoLocation',$ajax);
                break;
            case 7:
                $otherOpt       = array( 'dimensions'=> 'ga:date' );
...
$args = array(
	...
	'flex-height'    => true,
	'height'        => 200,
	'default-image' => 'http://affinitypartnerships.com/wp-content/uploads/2016/04/world-puzzle.gif',
);
