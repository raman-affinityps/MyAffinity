 ...
 * @license GPL 2.0
 * 
 * Template Name: Affiliate Dashboard Page
 */
...
if ( isset($_POST['db_action']) ) {
    $informa_tables = [
        "current_month_end"     => 'informaComparison_current',
        "current_month_mid"     => 'informaComparison_currentMid',
        "previous_month_end"    => 'informaComparison_previous',
        "previous_month_mid"    => 'informaComparison_previousMid',
    ];

    $clear_table = (isset($_POST['clear_table']) && $_POST['clear_table'] == 1) ? true : false;

    switch($_POST['db_action']) {
        case 'informa_upload':
            $which = $_POST['informa_which'];
            $itable = $informa_tables[$which];
            $q = "INSERT INTO `" . $itable . "` SET 
                `lender`='%s',
                `rate`='%s',
                `apr`='%s',
                `fees`='%s'";

            $file = (isset($_FILES['informa_file']) ? $_FILES['informa_file'] : null);
            $f = fopen($file['tmp_name'], 'r');

            fgetcsv($f, 1000);
            if ( $clear_table ) {
                $mysqli->query("truncate `" . $itable . "`");
            }

            while( ($fl = fgetcsv($f, 1000)) !== false ) {
                list($lender, $rate, $apr, $fees) = $fl;
                
                if ( empty($lender) ) continue;
                
                $qq = sprintf($q, $lender, $rate, $apr, trim($fees));

                $mysqli->query($qq);
            }
            fclose($f);

            $action_notice = "Informa Updated";

            break;

        case 'survey_upload':
            $q = "INSERT INTO `ap_survey` (CollectorID,StartDate,EndDate,IPAddress,Email,FirstName,LastName,Lender,src,OverallExperience,LenderPerformance,WouldRecommend,CanShare,Comments) 
            ...
            fgetcsv($f, 1000);
            if ( $clear_table ) {
                $mysqli->query("truncate `ap_survey`");
            }

            while( ($fl = fgetcsv($f, 1000)) !== false ) {
                list($CollectorID,$StartDate,$EndDate,$IPAddress,$Email,$FirstName,$LastName,$Lender,$src,$OverallExperience,$LenderPerformance,$WouldRecommend,$CanShare,$Comments) = $fl;
 
                $qq = sprintf($q, $CollectorID,$StartDate,$EndDate,$IPAddress,$Email,$FirstName,$LastName,$Lender,$src,$OverallExperience,$LenderPerformance,$WouldRecommend,$CanShare,$Comments);
                $mysqli->query($qq);
            }
            fclose($f);

            $action_notice = "Surveys Updated";
...
?>
    ...
    <h2 class="notice"><?php echo $action_notice; ?></h2>
...
h2.notice {
    font-size: 22px;
    line-height: 34px;
    background: rgba(220,125,25,0.5);
    padding: 20px;
    margin: 0 0 15px;
}
h2.notice:empty {
        ...
        <div id="content" class="site-content" role="main">
            ...
            <ul id="myTab" role="tablist" class="wpsm_nav wpsm_nav-tabs">
                <li class="active" role="presentation">
                    <a data-toggle="tab" role="tab" aria-controls="tabs_desc_1" href="#tabs_desc_1" aria-expanded="true"><i class="fa fa-columns"></i><span>Leads</span></a>
                </li>
                ...
                <li class="" role="">
                    <a data-toggle="tab" role="tab" ari-acontrols="tabs_desc_3" href="#tabs_desc_3" aria-expanded="false"><i class="fa fa-globe"></i> <span>Site Stats</span></a>
                </li>
<?php
    if ( $is_admin ) {
?>
                <li class="" role="">
                    <a data-toggle="tab" role="tab" ari-acontrols="tabs_desc_4" href="#tabs_desc_4" aria-expanded="false"><i class="fa fa-user"></i> <span>Administration</span></a>
                </li>
<?php
    }
?>
...
<?php
    if ( $is_admin ) {
?>
                <a href="/lender-dashboard/" onclick="document.location.href='/lender-dashboard/'">Lender Dashboard</a> |
 <?php
    }
?>
...
?>
            ...
            <div id="tabs_desc_4" class="tab-pane animated" role="tabpanel">
                <form action="/affiliate-dashboard/" method="post" enctype="multipart/form-data" style="margin-bottom:50px;display:block">
                    <input type="hidden" name="db_action" value="survey_upload">
                    <label for="csv_file" style="display:block;margin-bottom:10px">Survey CSV Data</label>
                    <input type="file" name="csv_file" id="csv_file" required><br>
                    <label style="display:block;margin:15px 0 10px"><input type="checkbox" name="clear_table" value="1"> Empty table before importing</label>
                    <input type="submit">
                </form>

                <form action="/affiliate-dashboard/" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="db_action" value="informa_upload">
                    <label for="informa_which" style="display:block;margin-bottom:10px">Informa CSV Data</label>
                    <select name="informa_which" id="informa_which">
                        <option value="current_month_end">Current Month End</option>
                        <option value="current_month_mid">Current Mid Month</option>
                        <option value="previous_month_end">Previous Month End</option>
                        <option value="previous_month_mid">Previous Mid Month</option>
                    </select>
                    <input type="file" name="informa_file" id="informa_file" required><br>
                    <label style="display:block;margin:15px 0 10px"><input type="checkbox" name="clear_table" value="1"> Empty table before importing</label>
                    <input type="submit">
                </form>
           </div>
